...
public final class TaskProcessSubjectPublisher implements ITaskProcessReceiver, ITaskProcessSubjectPublisher {
    ...
    public void taskStart(ComfyTaskStart start) {
        publishSubject(TaskProcessType.START, start.getTaskId(), start);
    }
    ...
    public void taskNodeProgress(ComfyTaskNodeProgress progress) {
        publishSubject(TaskProcessType.PROGRESS, progress.getTaskId(), progress);
    }
    ...
    public void taskProgressPreview(ComfyTaskProgressPreview preview) {
        publishSubject(TaskProcessType.PREVIEW, preview.getTaskId(), preview);
    }
    ...
    public void taskOutput(ComfyTaskOutput output) {
        publishSubject(TaskProcessType.OUTPUT, output.getTaskId(), output);
    }
    ...
    public void taskComplete(ComfyTaskComplete complete) {
        publishSubject(TaskProcessType.COMPLETE, complete.getTaskId(), complete);
    }
    ...
    public void taskError(ComfyTaskError error) {
        publishSubject(TaskProcessType.ERROR, error.getTaskId(), error);
    }
    ...
    public void taskNumberUpdate(ComfyTaskNumber taskNumber) {
        publishSubject(TaskProcessType.NUMBER_UPDATE, taskNumber);
    }
     ...
     * @param process   任务消息
     */
    private void publishSubject(TaskProcessType eventType, IComfyTaskProcess process) {
        publishSubject(eventType, null, process);
    }
     ...
     * @param process   任务消息
     */
    private void publishSubject(TaskProcessType eventType, String taskId, IComfyTaskProcess process) {
        TaskProcessMessage<?> subject = new TaskProcessMessage<>(eventType, taskId, process, this);
