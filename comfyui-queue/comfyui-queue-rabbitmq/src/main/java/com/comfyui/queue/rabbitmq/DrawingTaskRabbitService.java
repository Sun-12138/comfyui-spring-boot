package com.comfyui.queue.rabbitmq;
...
import com.comfyui.queue.rabbitmq.message.DrawingTaskInfoMessage;
...
public class DrawingTaskRabbitService implements IDrawingTaskSubmit, ITaskProcessReceiver {
    ...
    public void receiveTask(String msg, Channel channel, Message amqpMessage) throws IOException {
        try {
            ...
            try {
                //执行绘图任务
                taskPaintingExecutor.execDrawingTask(msgObj.getTaskInfo());
    ...
    private void sendTaskProgressToMonitorQueue(DrawingTaskProcessMessage processMessage) {
        amqpTemplate.convertAndSend(RabbitMonitorQueueConfig.TASK_MONITOR_EXCHANGE, "", processMessage.toString());
    }
